...
IOM_Nodes_Para_CSV_Data  s_Nodes_Para_DIAGNOSTIC_IOM_DATA[2],s_Nodes_Para_CHANNEL_CONFIGURATION_IOM_DATA[2];	
...
{
	modbusFrame->HPA_whole.HPA_DI_Per_Channel[ChannelNo].HPA.DI_bitAllocation.DI_Bits = 0x00;
	modbusFrame->HPA_whole.HPA_DI_Per_Channel[ChannelNo].HPA.DI_bitAllocation.bits.block = csv_data->block[ChannelNo];
	modbusFrame->HPA_whole.HPA_DI_Per_Channel[ChannelNo].HPA.DI_bitAllocation.bits.Inv = csv_data->Inversion[ChannelNo];
	modbusFrame->HPA_whole.HPA_DI_Per_Channel[ChannelNo].HPA.DI_bitAllocation.bits.flutter_ODS = csv_data->Flutt_dp_qds[ChannelNo];
	modbusFrame->HPA_whole.HPA_DI_Per_Channel[ChannelNo].HPA.DI_bitAllocation.bits.State_Of_flutter = csv_data->State_of_flutter_information[ChannelNo];
...
else if(csv_data->TI[ChannelNo] == DOUBLE_POINT_DI_TI)
{
	...
	modbusFrame->HPA_whole.HPA_DI_Per_Channel[ChannelNo].HPA.DI_bitAllocation.bits.block = csv_data->block[ChannelNo];
	modbusFrame->HPA_whole.HPA_DI_Per_Channel[ChannelNo].HPA.DI_bitAllocation.bits.Inv = csv_data->Inversion[ChannelNo];
	modbusFrame->HPA_whole.HPA_DI_Per_Channel[ChannelNo].HPA.DI_bitAllocation.bits.Inv_1 = csv_data->Inversion_1[ChannelNo];
	modbusFrame->HPA_whole.HPA_DI_Per_Channel[ChannelNo].HPA.DI_bitAllocation.bits.flutter_ODS = csv_data->Flutt_dp_qds[ChannelNo];
...
int extract_DI_Data_From_CSV(char IOM_Type, char IOM_Type_of_data,IOM_DI_CSV_Data *data)
{
	...
	if(IOM_Type == e_DI_IOM_TYPE)
	{
		...
		if(IOM_Type_of_data == e_DI_CHANNEL_CONFIGURATION_IOM_DATA)
		{
		while (fgets(buf, BUFSIZE, fp) != NULL) 
		{
	...
	count
			...
			else if(IOM_Type_of_data == e_DI_CHANNEL_DIAGNOSTIC_IOM_DATA || IOM_Type_of_data ==  e_DI_DIAGNOSTIC_IOM_DATA)
			{
				while (fgets(buf, BUFSIZE, fp) != NULL) 
				{
...
int extract_Nodes_Parameters_Data_From_CSV(char IOM_Type_of_data,IOM_Nodes_Para_CSV_Data *data,char *FileName)
{
	...
	if(IOM_Type_of_data == e_NODES_Para_DIAGNOSTIC_IOM_DATA)
	{

		string_to_compare = malloc(snprintf(NULL,0,"SW-")+1);
		sprintf(string_to_compare,"SW-");
		
		InputFile = malloc(snprintf(NULL,0,"%s_%02d",FileName, e_NODES_Para_DIAGNOSTIC_IOM_DATA+1)+1);


		sprintf(InputFile,"%s_%02d",FileName, e_NODES_Para_DIAGNOSTIC_IOM_DATA+1);				
	}
	else if(IOM_Type_of_data == e_NODES_Para_CONFIGURATION_RMValue_DATA)
	{
		string_to_compare = malloc(snprintf(NULL,0,"Ch-")+1);
		sprintf(string_to_compare,"Ch-");
		
		InputFile = malloc(snprintf(NULL,0,"%s_%02d",FileName, e_NODES_Para_CONFIGURATION_RMValue_DATA+1)+1);


		sprintf(InputFile,"%s_%02d",FileName, e_NODES_Para_CONFIGURATION_RMValue_DATA+1);
	}
	else if(IOM_Type_of_data == e_NODES_Para_CONFIGURATION_RBValue_DATA)
	{
		string_to_compare = malloc(snprintf(NULL,0,"Ch-")+1);
		sprintf(string_to_compare,"Ch-");
		
		InputFile = malloc(snprintf(NULL,0,"%s_%02d",FileName, e_NODES_Para_CONFIGURATION_RBValue_DATA+1)+1);


		sprintf(InputFile,"%s_%02d",FileName, e_NODES_Para_CONFIGURATION_RBValue_DATA+1);
	}
	else if(IOM_Type_of_data == e_NODES_Para_CONFIGURATION_RBValueWOProcessing_DATA)
	{
		string_to_compare = malloc(snprintf(NULL,0,"Ch-")+1);
		sprintf(string_to_compare,"Ch-");
		
		InputFile = malloc(snprintf(NULL,0,"%s_%02d",FileName, e_NODES_Para_CONFIGURATION_RBValueWOProcessing_DATA+1)+1);


		sprintf(InputFile,"%s_%02d",FileName, e_NODES_Para_CONFIGURATION_RBValueWOProcessing_DATA+1);
	}
	...
	else if(IOM_Type_of_data == e_NODES_Para_CONFIGURATION_RMValue_DATA)
	{
		while (fgets(buf, BUFSIZE, fp) != NULL) 
		{
			...
			if(token != NULL)	
			{				
				channel_no = GetChannelNumber(token);			
				strcpy(data->Name[channel_no], token);
			}
			...
			if(token != NULL)data->CASDU2[channel_no] = atoi(token);
			token = strsep(&p, ",\n");//4rd field
			if(token != NULL)data->Function_Code[channel_no] = atoi(token);
			token = strsep(&p, ",\n");//5th field
			if(token != NULL)data->IOA1[channel_no] = atoi(token);
			token = strsep(&p, ",\n");//6th field
			if(token != NULL)data->IOA2[channel_no] = atoi(token);
			...
			if(token != NULL)data->IOA3[channel_no] = atoi(token);
			token = strsep(&p, ",\n");//8th field
			if(token != NULL)data->Modbus_Address[channel_no] = atoi(token);
			token = strsep(&p, ",\n");//9th field
			if(token != NULL)data->Thresh_Additive[channel_no] = atoi(token);
			token = strsep(&p, ",\n");//10th field
			if(token != NULL)data->Thresh_Uncond[channel_no] = atoi(token);	
			token = strsep(&p, ",\n");//11th field
			if(token != NULL)data->Station_Number[channel_no] = atoi(token);
			token = strsep(&p, ",\n");//12th field
			if(token != NULL)data->TI[channel_no] = atoi(token);
			token = strsep(&p, ",\n");//13th field
			if(token != NULL)data->X_Zero[channel_no] = atof(token);
			token = strsep(&p, ",\n");//14th field
			if(token != NULL)data->X_100[channel_no] = atof(token);
			token = strsep(&p, ",\n");//15th field
			if(token != NULL)data->Y_Zero[channel_no] = atof(token);
			token = strsep(&p, ",\n");//15th field
			if(token != NULL)data->Y_100[channel_no] = atof(token);
			token = strsep(&p, ",\n");//16th field
			if(token != NULL)strcpy(data->Description[channel_no], token);
			...
			data->CASDU1[channel_no], 
			data->CASDU2[channel_no],
			data->Function_Code[channel_no],
			data->IOA1[channel_no], 
			data->IOA2[channel_no], 
			data->IOA3[channel_no],			
			data->Modbus_Address[channel_no],
			data->Thresh_Additive[channel_no],
			data->Thresh_Uncond[channel_no],
			data->Station_Number[channel_no],		
			data->TI[channel_no],
			data->X_Zero[channel_no],
			data->X_100[channel_no],
			data->Y_Zero[channel_no],
			data->Y_100[channel_no],
			data->Description[channel_no],
			channel_no,
			count
			);
	...
	else if(IOM_Type_of_data == e_NODES_Para_CONFIGURATION_RBValue_DATA || IOM_Type_of_data == e_NODES_Para_CONFIGURATION_RBValueWOProcessing_DATA)
	{
		while (fgets(buf, BUFSIZE, fp) != NULL) 
		{
...
uint8_t GetChannelNumber(char* Channel_number_string)
{
	...
	for(int i = 0; i < 250;i++)
	{	
		cmd = malloc(snprintf(NULL,0,"%s-%02d","Ch",i)+1);
		sprintf(cmd,"%s-%02d","Ch",i);
		if(strcmp(Channel_number_string, cmd) == 0) channel_no = i;
	}
	return channel_no;
}
