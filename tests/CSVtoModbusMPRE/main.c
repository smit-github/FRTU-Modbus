...
QuantityOfCSVData DataAvailablity;
uint8_t No_of_IOMs,No_of_Nodes,No_of_CPU,No_of_MPRE;
...
//	No_of_IOMs = SeperateCSV();
	...
	fetchCPUHeaderCSVandSeperate(&CPU_Header_data,&IOM_Header_data, &MPRE_Header_data,&DataAvailablity);

	printf("*************CSV Statictics**************\n");
	printf("	IOMs Available: %s\n", DataAvailablity.IOMAvailable ? "Yes" : "No");
	printf("	No. Of IOMs: %d\n", DataAvailablity.NoOfIOMs);
	printf("	CPUs Available: %s\n", DataAvailablity.CPUAvailable ? "Yes" : "No");
	printf("	No. Of CPUs: %d\n", DataAvailablity.NoOfCPUs);

	printf("	No. Of MPRES: %d\n", DataAvailablity.NoOfMPREs);


	printf("	MPRE1s Available: %s\n", DataAvailablity.MPREAvailable[e_MPRE_1] ? "Yes" : "No");


	printf("	MPRE2s Available: %s\n", DataAvailablity.MPREAvailable[e_MPRE_2] ? "Yes" : "No");


	if(DataAvailablity.MPREAvailable[e_MPRE_1] == 1 || DataAvailablity.MPREAvailable[e_MPRE_2] == 1)
	{
		for(i = 0; i< DataAvailablity.NoOfMPREs; i++)
		{	
		fetchNodesandSeperate(&MPRE_Header_data, &MPRE_Nodes_Header_data, i, &DataAvailablity);

		printf("	Nodes for MPRE%d Available: %s\n",i+1, 			DataAvailablity.NoOfNodesForMPRE_Available[i] ? "Yes" : "No");
		printf("	No. Nodes for MPRE%d: %d\n", i+1, DataAvailablity.NoOfNodesForMPRE[i]);
	...
	if(DataAvailablity.CPUAvailable == 1)
	{

		No_of_CPU = extract_CPU_MODULE_Data_From_CSV(&s_CPU_MODULE_CSV_Data);
		printf("\nNo. Of CPUs: %d\n", No_of_CPU);

		No_of_CPU = extract_CPU_DIAG_Data_From_CSV(&s_CPU_DIAGNOSTIC_DATA);
		printf("\nNo. Of CPUs: %d\n", No_of_CPU);
	}

	for(int j = 0; j< DataAvailablity.NoOfMPREs; j++)
	{
		for(i = 0; i< DataAvailablity.NoOfNodesForMPRE[j]; i++)
		{
			fetchNodesParametersandSeperate(i,j);

			sprintf(InputFile_whole,"MPRE%02d_Nodes_%02d",j+1,i+1);
			printf("File Name: %s\n", InputFile_whole);

			extract_Nodes_Parameters_Data_From_CSV(e_NODES_Para_DIAGNOSTIC_IOM_DATA, &s_Nodes_Para_DIAGNOSTIC_IOM_DATA[j],InputFile_whole);
			extract_Nodes_Parameters_Data_From_CSV(e_NODES_Para_CONFIGURATION_RMValue_DATA, &s_Nodes_Para_CHANNEL_CONFIGURATION_IOM_DATA[j],InputFile_whole);

			extract_Nodes_Parameters_Data_From_CSV(e_NODES_Para_CONFIGURATION_RBValue_DATA, &s_Nodes_Para_CHANNEL_CONFIGURATION_IOM_DATA[j],InputFile_whole);
			extract_Nodes_Parameters_Data_From_CSV(e_NODES_Para_CONFIGURATION_RBValueWOProcessing_DATA, &s_Nodes_Para_CHANNEL_CONFIGURATION_IOM_DATA[j],InputFile_whole);

		}
	}

	if(DataAvailablity.IOMAvailable == 1)
	{
		Recognize_IOMs(&IOM_Header_data,DataAvailablity.NoOfIOMs);
		for(i = 0; i< DataAvailablity.NoOfIOMs; i++)
		{
